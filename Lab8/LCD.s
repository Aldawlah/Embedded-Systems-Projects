...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
	PUSH {LR}
	BL waitForNotBusy				;	wait for busy bit to clear
    LDR R1, =GPIO_PORTA_DATA_R		;	clear D/C
	LDR R2, [R1]
	BIC R2, #0X40
	STR R2, [R1]
	LDR R1, =SSI0_DR_R				;	write command to SSIO_DR_R
	STR R0, [R1]
	BL waitForNotBusy				;	wait for busy bit to clear
	POP {LR}
    BX  LR                          ;   return
...
;4) Write the 8-bit data to SSI0_DR_R
	PUSH {LR}
	BL waitForTNFHigh				;	wait for TNF bit to go high
	LDR R1, =GPIO_PORTA_DATA_R		;	set D/C
	LDR R2, [R1]
	ORR R2, #0X40
	STR R2, [R1]
    LDR R1, =SSI0_DR_R				;	write data to SSIO_DR_R
	STR R0, [R1]
	POP {LR}
    BX  LR                          ;   return
